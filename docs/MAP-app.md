...
| 7514–8540 | **Penjahit production ledger** (ledger tabs, assignment wizard, penjahit and job pages, payments, order costs) | `Penjahit production ledger` | Production assignment, job pages, penjahit money |
...
**Availability probe** `rememberProductionReady` · `productionReadyGuess` · `applyProductionEntries` · **`productionReady`**

**Money** `productionBalance` · `productionTotals` · `productionTotalsHtml` · `productionJobHtml` (order page only)
...
**Ledger actions outside the region** `toggleCustomerDone` 1506 · `deletePenjahitById` 1525 (beside `deleteCustomerFromLedger`) · `swipeRowHtml` 892 · `bindSwipeRows` 761
...
- **The assignment flow is three screens, not one page.** `state.production.draft.step` drives `showProductionStep`, which toggles `[data-step]` panels and rewrites `#productionKicker` / `#productionTitle` from `PRODUCTION_STEPS`. Back is `#productionBack` in the fixed nav; `#productionBar` holds the one green forward button, labelled by `syncProductionBar`. Editing one existing job is `productionStepCount() === 1`: stepper and shared card hidden, step 3 only.
