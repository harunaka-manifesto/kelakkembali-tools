...
- **Every shortcut opens its list first** where the app has one. Quotation and Invoice go to `#/quotations?new=1` / `#/invoices?new=1`, Fitting to `#/fittings?new=1`, and each opens the create picker over its list only when that list is empty (`showFittingLogs` strips the flag and checks `fs.phase` / `fs.items` the same way the document feed does). Customer scrolls to the Customers ledger on this page and only goes to `#/customer/new/edit` when there are no customers. Order and Moodboard still open the picker directly: neither has a list page of its own, and the picker's first step is the customer list
...
- **Skeleton parity** `.home-skel__actions` must stay the exact size of the real row: 152px for the six, 208px once `body.has-production` adds the assignment row. `.home-skel__tabs` (57px) reserves the ledger tabs under the same flag.
- **Own-loader routes enter immediately.** `handleRoute` arms the page's enter fade before the data request; for the homepage, order, fitting feed, workspace, schedules and document feed it now also runs `enterView()` straight away. Before, those pages sat at opacity 0 until their data arrived, so their bespoke skeletons were painted but never seen — a slow load was a blank canvas and the progress bar The production placeholder is **last in the markup and `order: -1` in CSS**, so the six keep the `nth-child` positions the fitting-flag rules count on. A skeleton that is the wrong size makes the page jump as it loads, which is the same fault as any other layout shift
...
- **The form save bar does not ride the keyboard, and hides while it is up; every other fixed bottom bar rides it.** `.savebar` (customer, order and penjahit editors, and the wizard's `#productionBar`) has `transform: none`, and `body.is-keyboard-open .savebar` hides it. `measureVisualViewport` sets that class when a text field has focus *and* the visual viewport is more than 150px shorter than the tallest it has been at this width — focus alone is true with the keyboard dismissed, height alone is true when browser toolbars move. Parked behind the keyboard, the bar still showed on Chrome with the address bar at the bottom, floating on the empty strip Chrome leaves where its toolbar was. `.actionbar`, `.fitdet-bar`, `.fitadd-undo`, `.schedcal-monthbar` and the toast still carry `--keyboard-offset`; tests hold both halves
...
- **Route loader kinds** `LOADER_KINDS` in app.js maps a route to the silhouette `#routeLoader` draws: `cust` (customer page), `editor` (customer, order and penjahit editors, assignment wizard and job edit), `record` (penjahit and job pages, in the order page's classes), `fitdet`, `moodboard`, and `form` — the old card stack, kept only for calendar settings and enquiry, which still look like it. Each canvas is qualified by `:not([data-kind])` in pages.css; a bare class loses to shared.css's `.route-loader__canvas { display: flex }` and every canvas drew at once. The order page's own skeleton covers Items, Designs, Payments, Penjahit work (when production is on), Schedules and the delete row
- **Ledger add rows survive sticky hover.** `.btn--outline:hover` in shared.css loads later and ties on specificity; on touch, hover sticks after a tap and the cream add row went transparent — ink on black, an empty box. `body.is-homepage .btn--empty:hover` and `.cust-order-list .btn--empty:hover` pin the cream
...
- **`?new=1` lands on the list first.** The picker opens on top of it only when the list is empty — `openPickerIfEmpty` runs after `startDocumentFirstPage` resolves and returns unless `ds.phase` is `"ready"` with no items. The flag is stripped from the hash immediately, so Back and a reload never reopen the sheet. The explicit New button always opens the sheet, whatever the count
...
- **Penjahit detail**: fixed nav (back to the tab, edit icon), name and contact in the title band, a Money card with the green **Assign work** action directly under it, a job ledger (count and total in the summary rail, then the fitting feed's own search field and stage chips — `.fitlog-search-section` and `.fitlog-stages` reused, with Ongoing, Owed, Done and Cancelled chips that combine: none on is every job, any on is jobs matching any of them (`JOB_FILTERS`, `setJobFilter`); search runs over work, customer, item and order — and `.cust-order-card` records showing status, customer · item, owed or credit, and deadline), then **Delete penjahit** at the bottom on the customer page's danger row. `deletePenjahitById` refuses up front for a penjahit with jobs and says to archive instead; the database's restrict foreign key refuses it too
...
- **Assignment wizard**: Back in the fixed nav (Cancel on step 1), step kicker, title, a "For <penjahit>" line on steps 2 and 3 (and on single-job edit) so the step-1 answer travels with the later questions, and a three-bar stepper inside the title block, the step's cards, and the green bar whose label says what it carries ("Continue · 2 items selected", "Save 2 assignments · Rp 1.350.000"). Step 1 is `.pcard` radio cards; step 2 is one `.psource` card per order with its items as check rows — the subtitle reads "2 pieces · Rp4.500.000" from the item's quantity and price, and there is no side word, since the check and tint already say chosen — and a rail that turns green once anything in it is chosen; step 3 is a "Same for every item" card plus one `.pjob` card per item. Arriving from a penjahit page opens on step 2. Editing one existing job shows step 3 alone
