...
- **Shortcuts** **3 + 3, every tile 98px.** Customer (orange, the slot and colour Schedule used to hold), Order (neutral, opens the picker in `neworder` mode), Quotation (`?new=1`), then Invoice (`?new=1`), Moodboard (violet) and Fitting (pink). Moodboard and Order are `<button>`s because neither knows its destination until something is picked; Fitting is an `<a>` to the list whose click handler opens the picker instead, so the shortcut makes a log while the list stays reachable from the menu. One `repeat(3, 1fr)` grid, one section
- **Above the six, full width: Assign to penjahit** — same face, rail, and press as a tile, `grid-column: 1 / -1`. Hidden until the production probe answers; see [21](#21-penjahit-production-ledger)
...
- **app.js** region **7419–8029**, banner `Penjahit production ledger`. `showProductionRoute` **7672** (every production route but the homepage tab), `showHomepageLedger` 7495, `saveProductionDraft` 7777, `saveProductionPayment` 7882, `renderOrderProduction` 7933, `bindProductionEvents` 7965 — full index in [MAP-app.md](MAP-app.md)
- **HTML** `#viewPenjahit` 864, `#viewPenjahitEdit` 872, `#viewProductionEdit` 883, `#viewProductionJob` 899, `#orderProductionSection` 1138, `#homeLedgerTabs` 224 + `#homePenjahit` 238
- **CSS** `pages.css` 4912+ (`.production-*`, the ledger tabs, the summary toggle). `.home-action--production` sits with the other tiles at 398
