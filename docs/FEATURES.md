...
- **Shortcuts** **3 + 2 + 2.** Three coloured tiles — Schedule, Quotation (`?new=1`), Invoice (`?new=1`) — then two more at half width, Moodboard (violet) and Fitting (pink), then a neutral row of two: Add customer (`#/customer/new/edit`) and Add order (opens the picker in `neworder` mode). Moodboard and Add order are `<button>`s because neither knows its destination until something is picked. The grid is **six columns**, not three: it is the smallest track count both rows divide into, so row one spans two each and row two spans three
- **`SHOW_FITTING_SHORTCUT`** (app.js, near the domain constants) is the one thing to flip to stand the Fitting tile down again. It sets the tile's `hidden` **and** `body.has-fitting-shortcut`; with it off, Moodboard has the second row to itself, spans it as a full-width band and inherits Fitting's pink, so the row is never a colour short. Fittings themselves do not depend on it: `#/fittings`, the order page's fitting entry, and scheduled appointments all work either way
- **Narrow phones** at `max-width: 389px` the Add row drops to 15px type and 12px side padding. Without it "Add customer" wraps to two lines while "Add order" does not, and the pair reads lopsided. The homepage empty-state button also sets `width: auto` — `.btn--block`'s `width: 100%` plus its own 24px side margins put it 24px past the right edge and gave the whole page horizontal scroll
