...
- **Shortcuts** **3 + 3, every tile 98px.** Schedule, Quotation (`?new=1`), Invoice (`?new=1`), then Moodboard (violet), Fitting (pink) and Add order (neutral, opens the picker in `neworder` mode). Moodboard and Add order are `<button>`s because neither knows its destination until something is picked. One `repeat(3, 1fr)` grid, one section — the separate neutral row is gone with Add customer
- **Adding a customer is not here** it is the last row of the ledger, always, rendered by `renderCustomerList` for a full list as well as an empty one. It used to render only on the empty/no-match branch, which meant the shortcut tile was the sole entry point the moment the ledger had anything in it. One place, next to the customers it makes
...
- **`?new=1` opens the picker only on an empty list.** The flag is stripped from the hash immediately, but whether the sheet opens is decided in `openPickerIfEmpty` **after** `startDocumentFirstPage` resolves — the row count is not knowable before then, and opening earlier meant the picker focused its search field only for `focusRoute` to take focus back, which raised the software keyboard and dropped it again. A list with rows in it is what the user asked to see; the sheet would bury it. The explicit New button always opens the sheet, whatever the count
