...
- **app.js** region **7429–8351**, banner `Penjahit production ledger`. `showProductionRoute` **7878** (every production route but the homepage tab), `showProductionStep` 7651, `switchLedgerTab` 7569, `saveProductionDraft` 8042, `saveProductionPayment` 8153, `renderOrderProduction` 8204, `bindProductionEvents` 8236 — full index in [MAP-app.md](MAP-app.md)
- **HTML** `#viewPenjahit` 875, `#viewPenjahitEdit` 883, `#viewProductionEdit` 900 (three `[data-step]` panels), `#viewProductionJob` 947, `#orderProductionSection` 1186, `#productionBar` 1718, `#homeLedgerTabs` 235 + `#homePenjahit` 249
- **CSS** `pages.css` 4940+ (`.production-*`, `.pstep`, `.pchoice`, `.psource`, `.pjob`, the ledger tabs, the summary toggle). `.home-action--production` sits with the other tiles at 398
- **Data** `db.productionAvailable`, `listPenjahit`, `listProductionSources`, `savePenjahit`, `listProductionJobs`, `listProductionPayments`, `saveProductionJobs`, `updateProductionJob`, `recordProductionPayment` · tables `penjahit`, `production_jobs`, `production_payments` · view `production_job_feed` — see [DATABASE.md](DATABASE.md)
- **Everything is behind one probe.** `productionReady` asks the database once a session whether `penjahit` exists, remembers the answer in `localStorage`, and shows or stands down every `[data-production-entry]` element through `applyProductionEntries`. Before the migration runs, the feature is simply not there. Nothing else may toggle those elements
- **A job is one penjahit's work on one order item**, identified by `orders.items[].id` — a permanent id the `production_item_ids` trigger stamps and preserves through renaming and reordering. The same item can carry sewing with one penjahit and embroidery with another, so quantities across jobs are **not** an inventory limit
- **Money never nets across jobs.** Balance is `amount − paid + refunded`; positive is outstanding, negative is credit, and `productionTotals` aggregates the two separately. A penjahit holding a credit on a cancelled job must not appear to owe less on a live one. `productionTotalsHtml` prints Refunded and Credit only when they are non-zero — a column of Rp 0 rows makes a real credit harder to spot, not easier
- **Nothing about a job is edited in place.** A wrong payment is voided with a reason and replaced; a cancelled job states its final agreed charge, including zero, and keeps every payment against it as credit; a job with payment history cannot move to another penjahit at all
- **Saves are retry-safe from the client.** `state.production.batchRequest` and `state.production.paymentRequest` hold the exact payload of an unconfirmed save. A rejection carrying an error `code` came from the database and rolled back, so the payload is dropped; a response that never arrived keeps it and the retry resends the same ids, which the RPCs match and answer with the existing rows. While a payment is unconfirmed its fields lock but **Cancel does not**, or the only way out of an unanswered save would be to leave the page
- **The assignment flow is three screens.** Step 1 asks which penjahit, step 2 which items, step 3 what the work is and what it costs; `#productionBar` carries Back and a primary button whose label says what it will do and what it is carrying ("Save 3 assignments · Rp 18.000.000"). It began as one page holding a penjahit select, a 340px inner scroller listing every item in the studio, and a stack of full editors — three unrelated decisions competing for one scroll, inside a second scrollbar nobody could aim at. Arriving from a penjahit page opens on step 2, because step 1 is already answered
- **Selection is said three ways** — a filled mark, a green edge, and the word in the side column — so it survives colour blindness, a bright screen, and a screen reader. Items are grouped by customer and order under a sticky header, because "Bridal skirt" from four different weddings is four identical rows without it. Tapping a chosen item removes it
- **Step 3 is one shared card and a row per item.** Most batches are one description at one price across every piece, so typing it once and letting a row override it is the difference between one form and eight. `applyProductionShared` fills every row that has not been given its own answer; editing a row's description, price or deadline marks it custom and detaches it from the card for good. Quantity never detaches a row — it differs per item by nature. A row whose error lives inside its folded disclosure has that disclosure opened before focus is sent to it
- **The assignment flow never dead-ends.** Missing penjahit, customer, or order can each be made from inside it (`?from=production`), and every route back carries `?resume=1` so the half-built draft is still there. `state.production.draft` survives the detour; `state.production.createdCustomerId` is cleared whenever the draft resets, so an abandoned assignment cannot aim the next new order at the wrong customer
- **On the order page** `renderOrderProduction` prints the committed penjahit cost beside the item's existing estimate and flags an overrun in words as well as colour. **The estimate is never rewritten** by what a penjahit later agreed to — it is a promise already made to the customer
