...
const r=/^(\d{4})-(\d{2})-(\d{2})/.exec(String(e||""));if(!r)return"";const n=Number(r[3]),o=t[Number(r[2])-1];return o&&n?n+" "+o+" "+r[1]:""}
const isHeic=e=>/(?:heic|heif)$/i.test(e&&e.type||"")||/\.(?:heic|heif)$/i.test(e&&e.name||"");async function convertHeicToJpeg(e){
if("function"!=typeof window.heic2any)throw new Error("This HEIC photo cannot be read on this browser.");let t=await window.heic2any({blob:e,toType:"image/jpeg",quality:.92})
;if(Array.isArray(t)&&(t=t[0]),!t)throw new Error("Could not convert that HEIC photo.");return t}return{
...
},isHeic:isHeic,convertHeicToJpeg:convertHeicToJpeg,hashString:function(t){let e=2166136261;for(let r=0;r<t.length;r++)e^=t.charCodeAt(r),e=Math.imul(e,16777619);return e>>>0},mulberry32:function(t){
